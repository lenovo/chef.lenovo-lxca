...
resource_name :lxca_cabinet
provides :lxca_cabinet
...
action :filter_by_uuid do
  ...
  if new_resource.uuid.nil?
    raise "Attribute uuid is mandatory for the action filter_by_uuid"
  end
  @client.fetch_cabinet(["#{new_resource.uuid}"]).map do |cabinet|
    cabinet.instance_variables.each do |att|
      puts "#{att} - #{cabinet.instance_variable_get att}"
    end
  end
