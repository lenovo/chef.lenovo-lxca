...
resource_name :lxca_fan_muxes
provides :lxca_fan_muxes
...
action :filter_by_chassis do
  ...
  @client.discover_fan_muxes({:chassis => "#{new_resource.chassis}"}).map do |fan_mux|
    fan_mux.instance_variables.each do |att|
      puts "#{att} - #{fan_mux.instance_variable_get att}"
    end
  end
...
action :filter_by_uuid do
  ...
  @client.fetch_fan_muxes(["#{new_resource.uuid}"]).map do |fan_mux|
    fan_mux.instance_variables.each do |att|
      puts "#{att} - #{fan_mux.instance_variable_get att}"
    end
  end
