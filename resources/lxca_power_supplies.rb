...
resource_name :lxca_power_supplies
provides :lxca_power_supplies
...
action :filter_by_chassis do
  ...
  @client.discover_power_supplies({:chassis => "#{new_resource.chassis}"}).map do |power_supply|
    power_supply.instance_variables.each do |att|
      puts "#{att} - #{power_supply.instance_variable_get att}"
    end
  end
...
action :filter_by_uuid do
  ...
  @client.fetch_power_supplies(["#{new_resource.uuid}"]).map do |power_supply|
    power_supply.instance_variables.each do |att|
      puts "#{att} - #{power_supply.instance_variable_get att}"
    end
  end
