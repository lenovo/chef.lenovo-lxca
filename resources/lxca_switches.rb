...
resource_name :lxca_switches
provides :lxca_switches
...
action :filter_by_chassis do
  ...
  @client.discover_switches({:chassis => "#{new_resource.chassis}"}).map do |switch|
    switch.instance_variables.each do |att|
      puts "#{att} - #{switch.instance_variable_get att}"
    end
  end
...
action :filter_by_uuid do
  ...
  @client.fetch_switches(["#{new_resource.uuid}"]).map do |switch|
    switch.instance_variables.each do |att|
      puts "#{att} - #{switch.instance_variable_get att}"
    end
  end
